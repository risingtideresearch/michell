...
//!
//! The dipole *magnitude* comes from the prescribed strip closure `μ = 2U f_a`
//! (read qualitatively — see [`DIPOLE_WEIGHT_C`]). That closure is crude: a
//! doublet sheet's density is fixed by a hypersingular, non-local functional
//! of itself, so no exact pointwise closure exists. The *structure* below
//! (weight, θ-parity, additive separation) is exact either way.
