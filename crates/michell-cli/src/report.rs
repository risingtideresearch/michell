...
/// Data rows per index page (the index paginates beyond this).
pub const INDEX_ROWS_PER_PAGE: usize = 30;
...
pub struct Row {
    ...
    /// Heel angle to highlight on the GZ curve [deg] (e.g. peak righting arm);
    /// no marker when 0.
    pub mark_deg: f64,
...
pub fn build(title: &str, header: &[String], table: &[Vec<String>], rows: &[Row]) -> Vec<u8> {
    ...
    let rows_per_page = INDEX_ROWS_PER_PAGE;
    ...
    // Per-row pages, each back-linking to the index page it is listed on.
    let mut row_pages = Vec::with_capacity(rows.len());
    for (i, row) in rows.iter().enumerate() {
        let p = pdf.add_page(PAGE_W, PAGE_H);
        draw_row_page(&mut pdf, p, index_pages[i / rows_per_page], row);
        row_pages.push(p);
    }
...
fn draw_row_page(pdf: &mut Pdf, page: usize, index_page: usize, row: &Row) {
    // Header with a back-link to the index.
    {
        ...
        pg.text(PAGE_W - MARGIN - w, PAGE_H - 30.0, 9.0, LINK_BLUE, false, back);
        pg.link(
...
fn draw_gz(pdf: &mut Pdf, page: usize, rect: (f64, f64, f64, f64), row: &Row) {
    ...
    // Highlight marker (peak righting arm).
    if row.mark_deg.abs() > 1e-6 && row.mark_deg <= phi_max + 1e-6 {
        // Interpolate GZ at the marked heel.
        let phi = row.mark_deg;
